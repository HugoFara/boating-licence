...
quatre pays — **🇫🇷 France · 🇩🇪 Allemagne · 🇨🇭 Suisse · 🇳🇱 Pays-Bas** — derrière un seul
...
| 🇳🇱 **Pays-Bas** | wetten.overheid.nl / KOOP XML — **aucun droit d'auteur n'existe** sur la législation néerlandaise (Auteurswet art. 11) | Dérivées de la loi : le CBR ne publie aucun catalogue, seulement des examens types, qui ne sont **pas** ingérés |
...
# Pays-Bas — klein vaarbewijs (droit néerlandais : aucun droit d'auteur, Auteurswet art. 11)
python run.py build --country NL

# Actes de l'UE — bateaux de plaisance (catégories de conception), certificats, qualifications
...
| **Web** | `run.py web` | Regroupe chaque banque approuvée dans le site statique : un sous-bundle par pays (`web/ch/`, `web/de/`, `web/int/`, `web/nl/` ; la France via `run.py fr`), le tronc commun mutualisé (`web/questions.<base>.<lang>.json`), et les **paquets Anki** + fichiers **Moodle GIFT** par langue dans chaque bundle. |

## Les pays

Les quatre sont de premier rang : chacun correspond à un descripteur dans
...
[`docs/italy.md`](docs/italy.md) (italiano — prévu, pas encore réalisé). Les couches
supranationales sont dans [`docs/eu.md`](docs/eu.md), et l'architecture transversale
dans [`docs/scope.md`](docs/scope.md).
...
### 🇳🇱 Pays-Bas — Klein Vaarbewijs

Le **klein vaarbewijs** en deux niveaux : **KVB I** pour les rivières, canaux et lacs,
et **KVB II** pour « les autres eaux intérieures » — une liste légale (Westerschelde,
Oosterschelde, Waddenzee, Eems, Dollard, IJsselmeer, IJmeer, Markermeer sauf la
Gouwzee), pas une appréciation. Le permis est requis pour les bateaux de 15 à 25 m ou
tout bateau de moins de 15 m capable de dépasser 20 km/h (*Binnenvaartbesluit* art. 16).

- **La base juridique la plus nette du projet.** Là où l'Allemagne a une *limitation*
  du droit d'auteur et la France une *licence ouverte*, l'**Auteurswet art. 11** dit que
...
- **234 questions dérivées de la loi**, rédigées à partir de ces articles et passées
  par le gate de revue — vérifiées de manière adversariale contre le texte source
  cité, réparées quand un vérificateur les rejetait, et seulement alors
  **approuvées**. Elles ont leur propre audit
...
- **Notation par points pondérés, pas par blocs.** KVB I : 40 QCM, 60 min, 1–3 points
  ...
  pratique** — donc aucune étape `practical` n'est inventée.
- **Player :** le **🇳🇱 Nederland** de la countrybar ouvre `web/nl/`, avec une
  interface néerlandaise, le sélecteur de permis KVB I/II (chacun portant son format
  d'examen CBR et son chronomètre), les trois régions d'eaux, les étapes vers le
  permis, et les téléchargements Anki/GIFT. Les 209 questions portables alimentent
  aussi le tronc commun (`questions.cevni.nl.json`, `questions.universal.nl.json`).
- **Build :** `python run.py build --country NL` → `data/kb.nl.sqlite` ; les questions
  sont rédigées et vérifiées via `tools/subagent_draft.py` (draft → verify → ingest →
  apply), et `run.py web` regroupe `web/nl/`. Détails dans
...
## Actes de l'UE — la couche de droit de l'Union (`EU`)

Un second membre supranational (`src/countries/eu.py`), et un droit d'une autre nature :
ces actes ne disent pas qui doit céder le passage, ils régissent **le bateau, son
certificat et votre qualification**. Comme `INT`, il ne sert qu'au sourcing — et il
**n'ajoute aucune base** à l'arbre des régimes : une catégorie de conception n'est pas
un troisième code de circulation, c'est du contenu portable valable sous n'importe
lequel, donc les unités UE alimentent `universal`.

- **Directive 2013/53/UE (bateaux de plaisance)** — directement examinable : l'annexe I
  fixe les catégories de conception A–D par force du vent et hauteur significative de
  vague, plus le marquage CE et la plaque constructeur. La **directive (UE) 2016/1629**
  définit le certificat de navigation intérieure de l'Union ; la **directive (UE)
  2017/2397** explique pourquoi un certificat néerlandais, allemand ou français est
  reconnu dans toute l'Union.
- **Réutilisation explicite.** EUR-Lex autorise la réutilisation de ses documents
  juridiques à des fins commerciales et non commerciales (**décision 2011/833/UE**) ;
  textes consolidés en CC BY 4.0, métadonnées en CC0. Les **normes harmonisées EN ISO**
  restent, elles, des œuvres CEN/ISO vendues à l'unité : citées comme la directive les
  cite, jamais reproduites.
- **Un acte, 24 langues.** Les références émises sont **neutres linguistiquement**
  (`Directive 2013/53/EU art. 12`) et le tagueur associe *(acte, numéro d'article)* → thème,
  de sorte que la même disposition est la même unité dans chaque version. Vérifié en
  EN/NL/DE/FR.
- **Build :** `python run.py build --country EU` → **159 unités**. Détails dans
...
La **countrybar 🌍 / 🇫🇷 / 🇩🇪 / 🇨🇭 / 🇳🇱** bascule entre les players nationaux, chacun
réutilisant le même moteur avec ses propres règles d'examen. Le player propose aussi le
**paquet Anki** et le fichier **Moodle GIFT** pour la langue active en téléchargements
en un clic.

### Langues

L'interface du player est traduite en **français, allemand, italien, anglais et
néerlandais**, et le contenu des questions est construit par langue. Lorsque le droit
officiel d'un pays n'est pas publié dans une langue (par ex. l'anglais nulle part,
l'italien seulement en
